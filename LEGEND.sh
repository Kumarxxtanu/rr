...
cat <<EOL > .devcontainer/devcontainer.json
{
    ...
    "postStartCommand": "echo '*'; echo '*         @LEGENDXOPL Bot Running      *'; echo '*'; python /workspaces/$rr/$(basename "$PYTHON_FILE")",
    "postCreateCommand": "pip install pymongo python-telegram-bot pyTelegramBotAPI certifi && chmod +x /workspaces/$rr/*",
    "customizations": {
        "vscode": {
            "settings": {
...
git commit -m "Add devcontainer configuration for $rr with automatic script detection and watermark"
